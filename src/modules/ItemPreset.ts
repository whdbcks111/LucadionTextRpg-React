...
import { AttributeModifier, EquipmentType, StatType } from "./Internal";
    ...
    static list: ItemPresetObject[] = [
        ...
        },
        {
            ...
            onUse: (player, index) => {
                ...
                player.inventory.delayTask(() => {
                    player.sendMessage('[ 추위가 거의 안 느껴진다.. ]');
        ...
        },
        {
            ...
            onUse: (player, index) => {
                ...
                player.inventory.delayTask(() => {
                    player.sendMessage('[ 생명력이 좀 더 빨리 재생되는 느낌이다.. ]');
        ...
        },
        {
            ...
            onUse: (player, index) => {
                player.sendMessage('[ 열이 안느껴지는 느낌이다.. ]');
        ...
        },
        {
            ...
            onForge: (item, efficiency) => {
                ...
                item.attributeModifiers.push(new AttributeModifier(true, AttributeType.MOVE_SPEED, 1.2));
            },
            type: '단조',
            maxCount: 10
        },
        ...
        },
        {
            ...
            onForge: (item, efficiency) => {
                item.attributeModifiers.push(new AttributeModifier(false, AttributeType.ATTACK, Math.max(0, efficiency - 1) * 250));
                item.attributeModifiers.push(new AttributeModifier(false, AttributeType.MOVE_SPEED, Math.max(0, efficiency - 1) * 11));
                item.attributeModifiers.push(new AttributeModifier(false, AttributeType.CRITICAL_DAMAGE, Math.max(0, efficiency - 1) * 6));
                item.attributeModifiers.push(new AttributeModifier(false, AttributeType.DEFEND_PENETRATE, Math.max(0, efficiency - 1) * 210));
            },
            durability: 650,
            maxCount: 1
        },
        ...
        },
        {
            ...
            onUse: (player, index) => {
                ...
                player.sendMessage('[ 3분동안 Lv.10 마법 강화 효과와 Lv.20 마나 재생 효과가 지속됩니다. ]');
            },
            maxCount: 20
        },
        {
            ...
            onUse: (player, index) => {
                ...
                player.inventory.addItemCount(index, -1);
                player.sendMessage('[ ...! 검은 기운이 일어나 당신의 몸에 흡수되었다. ]');
            },
            maxCount: 20
        },
        {
            ...
            onUse: (player, index) => {
                ...
                player.sendMessage('[ ...! 붉은 기운이 일어나 당신의 몸에 흡수되었다. ]');
                player.inventory.addItemCount(index, -1);
            },
            maxCount: 1
        },
        {
            ...
            onUse: (player, index) => {
                if (player.log.getLog('gremory_heart') == 0) {
                    ...
                    player.inventory.addItemCount(index, -1);
                    player.sendMessage('[ ...! 검붉은 기운이 일어나 당신의 몸 주위를 돌아다닌다...! 어딘가에서 절규와 비명이 들리기 시작한다... ]');
                    setTimeout(() => player.sendMessage('[ ....... ]'), 1000);
                    setTimeout(() => {
                        player.sendMessage('[ ' + player.getName() + '님, 사망하셨, 하,. 지않으셨습니다. -1초 후에 부활합니다. ]');
                        player.realizeSkill('초에니 바르도', true);
                    }, 3000);
                }
                else player.sendMessage('[ 더 이상 필요 없다. ]');
            },
            maxCount: 20
        },
        {
            ...
            onUse: (player, index) => {
                if (!player.slot.hand) {
                    player.sendMessage('[ (들고 있는 무기가 없다...) ]');
                    return;
                }
                player.slot.hand.options.push(new Option('독날', { level: 10, time: 10 }, 60 * 5));
                player.inventory.addItemCount(index, -1);
                player.sendMessage('[ 검은 기운이 무기에 달라붙었다. ]');
            },
            maxCount: 20
        },
        {
        ...
        },
        {
            ...
            onUse: Item.CUSTOM_USE_EVENT({
                delay: 2, delayMessage: '[ 꿀꺽 꿀꺽... ]', run: (player, index) => {
                    player.sendMessage('[ Lv.2 재생이 부여되었습니다. ]');
        ...
        },
        {
            ...
            onUse: (player, index) => {
                ...
                player.addEffect(new Effect(EffectType.ENHANCE_MAGIC, 10, 3 * 60));
                player.sendMessage('[ 3분동안 마법 공격력이 50% 증가합니다. ]');
            },
            maxCount: 50
        },
        {
