...
import { Entity, FishDrops, ItemStack, Monster, Npc, Player, Resource, Terrain } from "./Internal";
import Enum from "./Enum";
import { ItemContainer } from "./Internal";
import { Shop } from "./Internal";
import { ShopPreset } from "./Internal";
import Utils from "./Utils";
import { LocationPresetObject, Lootable } from "../types";
...
export class Location {
    ...
    getPlayersInfo() {
        return '▌ 플레이어\n' +
            this.getPlayers(false)
                .map((p, idx) =>
                    `   ${idx + 1} > ${p.getName()}` +
                    '  ' + Utils.progressBar(6, p.life, p.maxLife, 'percent'))
                .join('\n');
    }
    ...
    getLocationInfo() {
        return `[ ${this.name}의 지형 정보 | ${this.zoneType.displayName} 지역` +
            (this.shop ? ' | 상점' : '') +
            (this.regionType === RegionType.WATER ? ' | 수중 지역' : '') +
            (this.fishDrops ? ' | 낚시 가능' : '') + ' ]\n\n' + Utils.rich +
            this.getMovableInfo() + Utils.blank + '\n\n' +
            '▌ 오브젝트\n' +
            this._objects.map((obj, idx) => {
                let out = `   ${idx + 1} > `;
                if (obj instanceof Monster) {
                    out += `[Lv.${obj.level}] ${obj.getName()}  ` +
                        (obj.isAlive ?
                            Utils.progressBar(6, obj.life, obj.maxLife, 'percent') :
                            '(죽음)' + (
                                obj.regenTime > 60 * 10 ?
            ...
            this.getPlayersInfo() + '\n\n' +
            '▌ 떨어진 아이템\n' +
            this.droppedItems.contents.map((itemStack, idx) => `   ${idx + 1} > ` +
                `${itemStack?.item?.getDisplayName()} x${itemStack?.count}`).join('\n');
    }
