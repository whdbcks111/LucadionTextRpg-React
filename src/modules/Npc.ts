...
import { Player, Time, Location } from "./Internal";
import { QuestPreset } from "./QuestPreset";
...
export class Npc {
    ...
    say(msg: string) {
        if(this.target) this.target.sendMessage(msg);
    }
    ...
    finish() {
        this.target?.sendMessage('[ 대화가 종료되었습니다. ]');
        this.target = null;
        this.choices = [];
        this.delayedTasks = [];
    }
...
export class Quest {
    ...
    giveReward(player: Player) {
        ...
        player.sendMessage(`[ 퀘스트 [${this.name}] 의 보상이 지급됩니다. ]`);
    ...
    update(player: Player) {
        ...
        if(this.canComplete(player) && !this.isCompleted) {
            ...
            player.sendMessage(`[ 퀘스트 [ ${this.name} ] (이)가 클리어 되었습니다! ]` +
