...
import { DropItem, Player } from "./Internal";
...
export class ResourcePreset {
    static list: ResourcePresetObject[] = [
        ...
        },
        {
            ...
            onDestroy: (resource, p) =>{
                    p.heal(3000);
                    p.sendMessage('[ 생명력이 3000만큼 회복되었다! ]\n' +
                        '생명력 ' + Utils.progressBar(6, p.life, p.maxLife, 'percent'));
            }
        },
        {
            name: '블러드 크리스탈',
            ...
            onDestroy: (resource, p) =>{
                if (p instanceof Player) {
                    let h = p.heal(20000);
                    p.sendMessage('[ 생명력이 ' + h.toFixed(1) + '만큼 회복되었다! ]\n' +
                        '생명력 ' + Utils.progressBar(6, p.life, p.maxLife, 'percent'));
                }
            }
        }
