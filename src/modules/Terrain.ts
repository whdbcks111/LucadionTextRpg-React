...
import { Item, ItemStack, Location, Player } from "./Internal";
...
export class Terrain {
    ...
    static ITEM_PICK_CHECK = (items: (ItemStack | null)[]) => (terrain: Terrain, p: Player) => {
        ...
        if(!item) return;
        p.sendMessage('[ ' + item.item + ' (을)를 ' + item.count + '개 획득했다! ]');
    ...
    static list: Terrain[] = [
        ...
        new Terrain('black_characters', '검은색 문자들', (terrain, p) => {
            p.sendMessage('[ 죽음이 두려운 자 중심에 서지 마라. 그렇지 않으면 용기를 보이되, 광휘의 힘으로 옳은 길로 가라. ]');
        }),
        new Terrain('life_shard', '라이프 샤드 (○)', (terrain, p) => {
            let loc = terrain.location;
            if(!loc) return;
            if (loc.movables.includes('인모르투 던전 - 2')) {
                p.sendMessage('[ 문은 이미 열렸다. ]');
                return;
            }
            let power = !!terrain.extras.power;
            if (!power) {
                p.sendMessage('[ 라이프 샤드가 켜졌습니다. 현재 생명력의 5%가 소멸했습니다. ]');
                p.life *= 0.95;
                terrain.name = '라이프 샤드 (●)';
            }
            else {
                p.sendMessage('[ 라이프 샤드가 꺼졌습니다. ]');
                terrain.name = '라이프 샤드 (○)';
            }
            ...
            if (isCorrect) {
                p.sendMessage('[ ..! (어딘가 열린 것 같다.) ]');
                p.sendMessage('[ 용기있는 자여, 앞으로 나아가거라. ]');
                setTimeout(() => {
