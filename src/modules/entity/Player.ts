import { AttributeType, EquipmentSlot, Stat, Attribute, Quest, Effect, Fish, Shield, EquipmentType, StatType, FishGrade, EffectType, MonsterType, Entity, User, ChatRoomManager, ChatRoom, Trigger, Config, ChatManager, Monster, Resource } from "../Internal";
...
export class Player extends LivingEntity {
    ...
    attack(victim: Entity, options: AttackOptions = {}): boolean {
        if (victim instanceof Monster && 
            !Array.from(victim.targets)
                .some(p => p instanceof Player && p.getPartyOwner() === this.getPartyOwner()) &&
            victim.targets.size > 0) {
            this.sendRawMessage('[ 다른 파티가 이미 싸우고 있습니다. ]');
            return false;
        }
        if (victim instanceof Resource && 
            victim.latestAbuser instanceof Player && 
            victim.latestAbuser.getPartyOwner() !== this.getPartyOwner() &&
            Date.now() - victim.latestAbused < 1000 * 10) {
            this.sendRawMessage('[ 다른 파티가 이미 파괴하고 있습니다. ]');
            return false;
        }
        let location = this.getLocation();
        if (victim instanceof Player && location.zoneType === ZoneType.PEACEFUL) {
            this.sendRawMessage('[ 평화지역에서는 싸울 수 없습니다. ]');
            return false;
        }
    ...
    onMove(from: string) {
        this.sendRawMessage('[ 이동을 완료했습니다. ]');
    }
    ...
    sendRawMessage(msg: string) {
        ChatManager.sendBotRawMessage(this.user.currentRoom, msg);
    }

    sendMessage(msg: MessageComponent) {
        ChatManager.sendBotMessage(this.user.currentRoom, msg);
    }
    ...
    saveData() {
        fs.writeFile(`${Utils.SAVE_PATH}players/${this.uid}.json`, JSON.stringify(this.toDataObj(), null, 4), () => {});
    }
    ...
    static saveAll() {
        const save = () => {
            if(!fs.existsSync(Utils.SAVE_PATH + 'players/')) fs.mkdirSync(Utils.SAVE_PATH + 'players/', { recursive: true });
            Player.players.forEach(player => {
                player.saveData();
            });
        }
        if(!fs.existsSync(Utils.TMP_PATH)) fs.mkdirSync(Utils.TMP_PATH, { recursive: true });
        if(fs.existsSync(Utils.SAVE_PATH + 'players/')) fs.rename(Utils.SAVE_PATH + 'players/', Utils.TMP_PATH + 'players', err => {
    ...
    static loadPlayer(uid: string) {
        let data = fs.readFileSync(`${Utils.SAVE_PATH}players/${uid}.json`).toString();
    ...
    static loadAll() {
        if(!fs.existsSync(Utils.SAVE_PATH + 'players/')) fs.mkdirSync(Utils.SAVE_PATH + 'players/', { recursive: true });
        let files = fs.readdirSync(Utils.SAVE_PATH + 'players/');
