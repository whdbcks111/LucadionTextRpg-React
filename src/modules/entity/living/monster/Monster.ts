...
export class Monster extends LivingEntity {
    ...
    tryAttack() {
        ...
        if(target && this.isAttackEnded) return this.attack(target);
        return false;
    }
    ...
    update() {
        ...
        if(this.deadTime <= 0 && this.onUpdate) this.onUpdate(this);
        if(this.isAlive) {
            ...
            if(!this.currentTarget) this.life += this.maxLife * 0.1 * Time.deltaTime;
        }
    ...
    lateUpdate() {
        ...
        let attackSpeed = this.attribute.getValue(AttributeType.ATTACK_SPEED);
        if (attackSpeed > 1.5) {
            this.attribute.multiplyValue(AttributeType.ATTACK_SPEED, 1.5 / attackSpeed);
            if (this.stat.getStat(StatType.SPELL) > this.stat.getStat(StatType.STRENGTH))
                this.attribute.multiplyValue(AttributeType.MAGIC_ATTACK, attackSpeed / 1.5);
            else {
                this.attribute.multiplyValue(AttributeType.ATTACK, attackSpeed / 1.5);
                this.attribute.multiplyValue(AttributeType.RANGE_ATTACK, attackSpeed / 1.5);
            }
        }
    }
    ...
    rewardPlayer(player: Player): void {
        ...
        let partyPlayers = Array.from(this.targets).concat()
        ...
        if (partyPlayers.length <= 1) {
            ...
            let maxExp = Entity.getMaxExp(partyPlayers.map(p => p.level).reduce((a, b) => Math.max(a, b), 1));
            partyPlayers.forEach(p => {
                let addition = gettingExp * Math.min(1, p.maxExp / maxExp + 0.05);
                p.exp += addition;
