...
import { Time, Utils } from "../Internal";
...
export class Inventory extends ItemContainer {
    ...
    getContentsInfo() {
        ...
        return ComponentBuilder.message([
            ComponentBuilder.text(`[ ${this.owner?.name} 님의 인벤토리 ]\n`),
            ComponentBuilder.text(`공간 [${this.contents.filter(is => is).length}/${this.contents.length}]`),
