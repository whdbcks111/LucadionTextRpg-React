...
import { Npc, NpcChoice, FishDrops, Monster, Terrain, Resource, ShopPreset, Item, ItemStack, ZoneType, Utils } from "../Internal";
...
export class World {
        ...
        npcs: [
            new Npc('배고픈 거지', [
                ...
                npc => {
                    ...
                    npc.target.sendRawMessage('[ (그것은 누군가에겐 \'맛있는 빵\'이었다.) ]');
                    npc.target.exp += 600;
                }
            ])
        ],
    ...
    static isValidLocationName(name: string) {
        return World.getLocationOrNull(name) !== null;
    }

    static getLocationOrNull(name: string): Location | null {
        let loc = cache[name] ?? (cache[name] = World.locations.find(loc => loc.name === name));
        return loc ?? null;
    }

    static getLocation(name: string): Location {
        let loc = World.getLocationOrNull(name);
