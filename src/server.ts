...
import {
    ...
    RegisterInfo, ServerPingData, PingRoomData, RegisterMessage, LoginMessage, ChatFlag, MessageComponent, CCUInfo, BattleBalanceInfo
} from './types';
...
const commandManager = new CommandManager([
    ...
    new Command(['?', '도움말', '명령어'], [], [],
        ...
        (chat, player, label, args) => {
            player.sendMessage(ComponentBuilder.message([
                ComponentBuilder.text(`[ 도움말입니다. ]`),
                ComponentBuilder.hidden([
                    commandManager.getCommandsInfo(player.isDev)
                ])
            ]));
        }).setAliveOnly(false),

    new Command(['실행'], [[CommandArg.STRING, '코드']], ['exec'],
        `관리자 권한으로 JS 코드를 실행합니다.`,
        (chat, player, label, args) => {
            try {
    ...
    new Command(['공지'], [[CommandArg.STRING, '공지글']], ['ann'],
        `짧은 메시지를 전체 방에 공지합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['스테이터스'], [], ['s'],
        `플레이어의 상태 정보를 확인할 수 있습니다.`,
        (chat, player, label, args) => {
            player.sendMessage(player.getStatusInfo());
        }),

    new Command(['로그인'], [], ['li', 'in'],
        `플레이어를 로그인 상태로 변경합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['로그아웃'], [], ['lo', 'ou'],
        `플레이어를 로그아웃 상태로 변경합니다.`,
        (chat, player, label, args) => {
            if (!player.isLoggedIn) {
    ...
    new Command(['맵'], [], ['m'],
        `현재 위치한 장소의 정보를 출력합니다.`,
        (chat, player, label, args) => {
            player.sendMessage(player.getLocation().getLocationInfo());
        }),

    new Command(['이동'], [[CommandArg.POSITIVE_INTEGER, '이동 가능한 장소 번호']], ['mv', 'v', 'go'],
        `현재 장소에서 이동할 수 있는 장소로 이동합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['칭호 사용'], [[CommandArg.INTEGER, '칭호 번호']], ['tu'],
        `보유한 칭호를 장착합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['랭킹', '순위'], [], ['rk'],
        `플레이어들의 랭킹 정보를 출력합니다.`,
        (chat, player, label, args) => {
            player.sendMessage(Player.getRankingInfo());
        }).setAliveOnly(false),

    new Command(['랭킹', '순위'], [[CommandArg.STRING, '랭킹 기준']], ['rk'],
        `플레이어들의 특정 기준 랭킹 정보를 출력합니다.`,
        (chat, player, label, args) => {
            try {
    ...
    new Command(['상점', '상점 목록'], [], ['sh', 'z'],
        `현재 위치한 장소가 상점이라면, 상점에서 거래 가능한 아이템의 정보를 출력합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['맵 플레이어'], [], ['mp'],
        `현재 위치한 장소에 있는 플레이어들을 출력합니다.`,
        (chat, player, label, args) => {
            let loc = player.getLocation();
            player.sendMessage(ComponentBuilder.message([
    ...
    new Command(['맵 장소'], [], ['ml'],
        `현재 위치한 장소에서 이동할 수 있는 다른 장소를 출력합니다.`,
        (chat, player, label, args) => {
            let loc = player.getLocation();
            player.sendRawMessage('[ 맵 갈 수 있는 장소 목록 ]\n' + loc.getMovableInfo());
        }),

    new Command(['스킬 목록'], [], ['sl', 'l'],
        `보유한 스킬들의 간략한 정보를 출력합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['스킬 정보'], [[CommandArg.STRING, '이름']], ['si', 'k'],
        `특정 스킬의 자세한 정보를 출력합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['구매'], [[CommandArg.POSITIVE_INTEGER, '상점 아이템 번호'], [CommandArg.POSITIVE_INTEGER, '개수']], ['bu'],
        `현재 위치한 상점의 아이템을 구매합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            if (!location.shop) {
                player.sendRawMessage('[ 이 장소는 상점이 아닙니다. ]');
    ...
    new Command(['판매'], [[CommandArg.POSITIVE_INTEGER, '상점 아이템 번호'], [CommandArg.POSITIVE_INTEGER, '개수']], ['se'],
        `현재 위치한 상점에 보유한 아이템을 판매합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            if (!location.shop) {
                player.sendRawMessage('[ 이 장소는 상점이 아닙니다. ]');
    ...
    new Command(['버리기'], [[CommandArg.POSITIVE_INTEGER, '인벤토리 슬롯 번호'], [CommandArg.POSITIVE_INTEGER, '개수']], ['dr', 'q'],
        `인벤토리 내의 아이템을 현재 위치한 장소에 버립니다.`,
        (chat, player, label, args) => {
            let index = parseInt(args[0]) - 1;
            let count = parseInt(args[1]);
    ...
    new Command(['줍기'], [[CommandArg.POSITIVE_INTEGER, '버려진 아이템 번호'], [CommandArg.POSITIVE_INTEGER, '개수']], ['p', 'j'],
        `현재 장소에 버려진 아이템을 줍습니다.`,
        (chat, player, label, args) => {
            let index = parseInt(args[0]) - 1;
            let count = parseInt(args[1]);
    ...
    new Command(['판매 전체'], [], ['sa'],
        `소유한 아이템 중 현재 위치한 상점에 팔 수 있는 모든 것을 판매합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            if (!location.shop) {
                player.sendRawMessage('[ 이 장소는 상점이 아닙니다. ]');
    ...
    new Command(['스탯 분배'], [[CommandArg.STRING, '스탯이름'], [CommandArg.POSITIVE_INTEGER, '개수']], ['st', 'r'],
        `잔여 스탯 포인트를 특정 스탯에 분배하여 능력치를 올립니다.`,
        (chat, player, label, args) => {
            let count = parseInt(args[1]);
    ...
    new Command(['장착 해제'], [[CommandArg.STRING, '장착 슬롯명']], ['dm'],
        `장착한 장비를 해제하여 인벤토리로 되돌립니다.`,
        (chat, player, label, args) => {
            let slotName = args[0];
    ...
    new Command(['제작'], [[CommandArg.STRING, '아이템 이름']], ['cr'],
        `해당하는 아이템을 제작합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['사용'], [[CommandArg.POSITIVE_INTEGER, '인벤토리 슬롯 번호']], ['u'],
        `번호에 해당하는 아이템을 사용합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['대화'], [[CommandArg.POSITIVE_INTEGER, 'NPC 번호']], ['t'],
        `번호에 해당하는 NPC와 대화를 시작합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
    ...
    new Command(['대화선택'], [[CommandArg.POSITIVE_INTEGER, '대화 선택지 번호']], ['cc'],
        `대화 중 선택지를 선택합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
    ...
    new Command(['확인'], [[CommandArg.POSITIVE_INTEGER, '지형 번호']], ['f', 'ck'],
        `번호에 해당하는 지형을 조사합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
    ...
    new Command(['감정'], [[CommandArg.STRING, '장착 슬롯 이름 / 인벤토리 슬롯 번호']], ['iid'],
        `해당하는 아이템의 정보를 출력합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['대상지정'], [[CommandArg.POSITIVE_INTEGER, '오브젝트 번호']], ['tg'],
        `번호에 해당하는 오브젝트를 타겟으로 지정합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
    ...
    new Command(['대상지정p'], [[CommandArg.POSITIVE_INTEGER, '플레이어 번호']], ['tgp'],
        `현재 장소에서 번호에 해당하는 플레이어를 타겟으로 지정합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
    ...
    new Command(['공격'], [[CommandArg.POSITIVE_INTEGER, '오브젝트 번호']], ['a'],
        `번호에 해당하는 오브젝트(몬스터, 자원)을 공격합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
            let object = location.objects[index];
    ...
    new Command(['공격p'], [[CommandArg.POSITIVE_INTEGER, '플레이어 번호']], ['ap'],
        `현재 위치한 장소에서 번호에 해당하는 플레이어를 공격합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
    ...
    new Command(['파티 초대'], [[CommandArg.POSITIVE_INTEGER, '플레이어 번호']], ['pi'],
        `현재 위치한 장소에서 번호에 해당하는 플레이어를 자신의 파티에 초대합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
    ...
    new Command(['파티 추방'], [[CommandArg.POSITIVE_INTEGER, '파티원 번호']], ['pk'],
        `번호에 해당하는 파티원을 파티에서 추방합니다.`,
        (chat, player, label, args) => {
            if (!player.partyOwner) {
                let index = parseInt(args[0]) - 1;
    ...
    new Command(['파티 정보'], [], ['pt'],
        `현재 참가한, 혹은 자신이 파티장인 파티의 정보를 출력합니다.`,
        (chat, player, label, args) => {
            player.sendRawMessage(player.getPartyInfo());
        }),

    new Command(['거래 요청'], [[CommandArg.POSITIVE_INTEGER, '플레이어 번호']], ['tr'],
        `현재 위치에서 번호에 해당하는 플레이어에게 거래를 요청합니다.`,
        (chat, player, label, args) => {
            let location = player.getLocation();
            let index = parseInt(args[0]) - 1;
            let target = location.getPlayers()[index];
    ...
    new Command(['거래 수락'], [], ['ta'],
        `받은 거래 요청을 수락하고 거래를 시작합니다.`,
        (chat, player, label, args) => {
            let request = TradeManager.getRequestTo(player);
            if (!request)
                player.sendRawMessage('[ 받은 거래 요청이 없습니다. ]');
    ...
    new Command(['거래 취소'], [], ['tce'],
        `현재 진행중인 거래를 취소하고 추가한 골드와 아이템을 모두 회수합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['거래 확인'], [], ['tc', 'tcf'],
        `거래 내용을 확인하고 거래를 종료할 준비를 합니다.`,
        (chat, player, label, args) => {
            let trade = TradeManager.getTrade(player);
            if (!trade)
                player.sendRawMessage('[ 거래가 시작되지 않았습니다. ]');
    ...
    new Command(['거래 아이템추가'], [[CommandArg.POSITIVE_INTEGER, '인벤토리 슬롯 번호'], [CommandArg.POSITIVE_INTEGER, '개수']], ['tia'],
        `현재 진행중인 거래 내용에 보유한 아이템을 추가합니다.`,
        (chat, player, label, args) => {
            let trade = TradeManager.getTrade(player);
            if (!trade)
                player.sendRawMessage('[ 거래가 시작되지 않았습니다. ]');
    ...
    new Command(['거래 아이템회수'], [[CommandArg.POSITIVE_INTEGER, '거래 아이템 슬롯 번호'], [CommandArg.POSITIVE_INTEGER, '개수']], ['tis'],
        `거래 내용에서 자신이 추가한 아이템을 회수합니다. 한 명이 이미 확인한 거래라면 회수가 불가능합니다.`,
        (chat, player, label, args) => {
            let trade = TradeManager.getTrade(player);
            if (!trade)
                player.sendRawMessage('[ 거래가 시작되지 않았습니다. ]');
    ...
    new Command(['거래 골드추가'], [[CommandArg.POSITIVE_INTEGER, '금액']], ['tga'],
        `현재 진행중인 거래 내용에 골드를 추가합니다.`,
        (chat, player, label, args) => {
            let trade = TradeManager.getTrade(player);
            if (!trade)
                player.sendRawMessage('[ 거래가 시작되지 않았습니다. ]');
    ...
    new Command(['거래 골드회수'], [[CommandArg.POSITIVE_INTEGER, '금액']], ['tgs'],
        `거래 내용에 추가한 골드를 회수합니다. 한 명이 이미 확인한 거래라면 회수가 불가능합니다.`,
        (chat, player, label, args) => {
            let trade = TradeManager.getTrade(player);
            if (!trade)
                player.sendRawMessage('[ 거래가 시작되지 않았습니다. ]');
    ...
    new Command(['단축키', '단축키 목록'], [], ['kb'],
        `명령어들의 단축키 정보를 출력합니다.`,
        (chat, player, label, args) => {
            player.sendMessage(ComponentBuilder.message([
                ComponentBuilder.text(`[ 단축키 목록입니다. ]`),
                ComponentBuilder.hidden([
                    commandManager.getKeyBindsInfo(player.isDev)
                ])
            ]));
        }).setAliveOnly(false),

    new Command(['동시접속', '동접'], [], ['con'],
        `현재 서버에 접속한 유저를 확인합니다.`,
        (chat, player, label, args) => {
    ...
    new Command(['개발자 인증'], [], [],
        `개발자임을 인증해 관리자 권한 기능을 사용할 수 있습니다.`,
        (chat, player, label, args) => {
...
chat.on('connection', (client: Socket) => {
    ...
    let user = getUser();
    if (!user) client.emit('login-require', '접속 시 유저 없음');
    else {
        client.join(user.currentRoom);
    }

    client.on('require-ccu-infos', () => {
        let user = getUser();
        if (!user) {
            client.emit('login-require', '동접자 불러올 시 유저 없음');
            return;
        }

        if(!user.player?.isDev) {
            client.emit('navigate', '/');
            return;
        }

        const ccuInfos: CCUInfo[] = Player.getConnectedPlayers()
            .map(p => {
                return {
                    nickname: p.name,
                    currentRoomId: p.user.currentRoom,
                    currentRoomName: p.user.room?.name ?? '',
                    locationName: p.location,
                    isDead: p.isDead,
                    lifeProgress: p.life / p.maxLife
                };
            });

        client.emit('ccu-infos', ccuInfos);
    });

    client.on('require-battle-balance-infos', (levelRange: [number, number]) => {
        let user = getUser();
        if (!user) {
            client.emit('login-require', '전투 밸런스 불러올 시 유저 없음');
            return;
        }
        ...
        const classPlayers: {[key: string]: Player[]} = {};
        const bbInfos: BattleBalanceInfo[] = [];

        Player.players.forEach(p => {
            const className = p.characterClass ?? '무직';
            if(levelRange[0] <= p.level && p.level <= levelRange[1]) {
                if(!(className in classPlayers)) {
                    classPlayers[className] = [];
                }
                classPlayers[className].push(p);
            }
        });

        function getAvgAttr(players: Player[], attrType: AttributeType) {
            return players
                .map(p => p.attribute.getValue(attrType))
                .reduce((a, b) => a + b, 0);
        } 

        for(let className in classPlayers) {
            const players = classPlayers[className];
            bbInfos.push({
                characterClassName: className,
                avgDamage: Math.max(
                    getAvgAttr(players, AttributeType.ATTACK),
                    getAvgAttr(players, AttributeType.RANGE_ATTACK),
                    getAvgAttr(players, AttributeType.MAGIC_ATTACK),
                ) / players.length,
                avgDefend: getAvgAttr(players, AttributeType.DEFEND),
                avgMagicResistance: getAvgAttr(players, AttributeType.MAGIC_RESISTANCE),
                avgMaxLife: getAvgAttr(players, AttributeType.MAX_LIFE),
                avgMoveSpeed: getAvgAttr(players, AttributeType.MOVE_SPEED)
            });
        }

        client.emit('battle-balance-infos', bbInfos);
    });

    client.on('previous-chats', () => {
        let user = getUser();
        if (!user) {
            client.emit('login-require', '이전 채팅 불러올 시 유저 없음');
            return;
        }
    ...
    const pingId = setInterval(() => {
        const user = getUser();
        if (!user) {
            client.emit('login-require', '핑 보낼 시 유저 없음');
            return;
        }
    ...
    client.on('change-profile', (base64: string) => {
        let user = getUser();
        if (!user) {
            client.emit('login-require', '프로필 변경 시 유저 없음');
            return;
        }
    ...
    client.on('change-room', (id: string) => {
        let user = getUser();
        if (!user) {
            client.emit('login-require', '방 변경 시 유저 없음');
            return;
        }
    ...
    client.on('chat', (data: ChatData) => {
        const user = getUser();
        if (!user) {
            client.emit('login-require', '채팅 시 유저 없음');
            return;
        }
